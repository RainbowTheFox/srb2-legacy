...
//	size_t q;
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		WRITEUINT8(save->p, players[i].charability);
		WRITEUINT8(save->p, players[i].charability2);
		WRITEUINT32(save->p, players[i].charflags);
		WRITEUINT32(save->p, (UINT32)players[i].thokitem);
		WRITEUINT32(save->p, (UINT32)players[i].spinitem);
		WRITEUINT32(save->p, (UINT32)players[i].revitem);
		WRITEFIXED(save->p, players[i].actionspd);
		WRITEFIXED(save->p, players[i].mindash);
		WRITEFIXED(save->p, players[i].maxdash);
		WRITEFIXED(save->p, players[i].normalspeed);
		WRITEFIXED(save->p, players[i].runspeed);
		WRITEUINT8(save->p, players[i].thrustfactor);
		WRITEUINT8(save->p, players[i].accelstart);
		WRITEUINT8(save->p, players[i].acceleration);
		WRITEFIXED(save->p, players[i].jumpfactor);
	}
}
...
static void P_NetUnArchivePlayers(savebuffer_t *save)
{
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		players[i].charability = READUINT8(save->p);
		players[i].charability2 = READUINT8(save->p);
		players[i].charflags = READUINT32(save->p);
		players[i].thokitem = (mobjtype_t)READUINT32(save->p);
		players[i].spinitem = (mobjtype_t)READUINT32(save->p);
		players[i].revitem = (mobjtype_t)READUINT32(save->p);
		players[i].actionspd = READFIXED(save->p);
		players[i].mindash = READFIXED(save->p);
		players[i].maxdash = READFIXED(save->p);
		players[i].normalspeed = READFIXED(save->p);
		players[i].runspeed = READFIXED(save->p);
		players[i].thrustfactor = READUINT8(save->p);
		players[i].accelstart = READUINT8(save->p);
		players[i].acceleration = READUINT8(save->p);
		players[i].jumpfactor = READFIXED(save->p);
	}
}
...
//
static inline void LoadLightlevelThinker(savebuffer_t *save, actionf_p1 thinker)
{
...
static void P_NetArchiveMisc(savebuffer_t *save)
{
	UINT32 pig = 0;
	INT32 i;
	...
	WRITEUINT32(save->p, countdown);
	WRITEUINT32(save->p, countdown2);
...
static inline boolean P_NetUnArchiveMisc(savebuffer_t *save)
{
	UINT32 pig;
	INT32 i;
	...
	countdown = READUINT32(save->p);
	countdown2 = READUINT32(save->p);
